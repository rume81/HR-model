...
public class SalarySheetMapper extends BaseMapper implements RowMapper<HSalarySheet> {

    public HSalarySheet mapRow(ResultSet rs, int rowNum) throws SQLException {
	HSalarySheet ob = new HSalarySheet();
	...
	if (colindex > 0) {
	    ...
	    if (null != jod)
		ob.setJoin_date(DateFormate().format(jod));
	    else
		ob.setJoin_date("");
	}
	...
	if (colindex > 0) {
	    ob.setDesignation(rs.getString(colindex));
	}
